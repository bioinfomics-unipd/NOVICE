...
#sys.argv[9] provide filename .tsv with functions


if len(sys.argv) < 6:
    print('----------------------------------------------')
    print('----------------------------------------------')
    print('CORRECT SYNTAX: NOVICE_1_1_5.py path/file argument1 argument2 argument3 argument4 argument5 argument6[required only if GC is chosen as option in argument1] argument7 argument8')
    ...
    print('available options for argument7: filename .tsv')
    print('----------------------------------------------')
    print('----------------------------------------------')
    sys.exit()
...
print('*******************************************************')
print('>>>>>>>>>>>>>> NOVICE version 1.1.5 beta <<<<<<<<<<<<<<<<<<<')
print('*******************************************************')
...
if sys.argv[2] == 'rank' or sys.argv[2] == 'all' or sys.argv[2] == 'PCA':
    ...
    function_data = ReadFiles(sys.argv[9])
    
    function_data.index = function_data['locus_tag']
    
    data_SORTED.index = data_SORTED['index']
    
    data_SORTED['Functions'] = ['Not Available' for i in data_SORTED['index']]
    
    for e in list(function_data['locus_tag']):
    	if e in list(data_SORTED['index']):
    		data_SORTED.loc[e, 'Functions'] = function_data.loc[e, 'product']
    		
    data_SORTED.to_csv(str(sys.argv[5])+ '_sorted_dataframe.csv')
    
    
    print('dataframe sorted is overwritten with function information')
...
if sys.argv[2] == 'Kmers' or sys.argv[2] == 'all':
    ...
    summa = [np.sum(data_SORTED.loc[e,data_SORTED.columns[16]:]) for e in data_SORTED['index']]
    
    print('Kmers frequency calculations can take a bit')
    print('Please wait...')
    ...
    i = 0
    for s in summa:
    	for e in range(len(data_SORTED.columns)-17):
    		data_SORTED.loc[data_SORTED.index[i], data_SORTED.columns[17+e]] = data_SORTED.loc[data_SORTED.index[i], data_SORTED.columns[17+e]]/s
    	if i != len(data_SORTED.index)-1:
    		i += 1
    ...
    pipeline = make_pipeline(scalar, pca)
    pipeline.fit(data_SORTED.iloc[:, 17:])
    ...
    model.fit(data_SORTED.iloc[:, 17:])
    transformed = model.transform(data_SORTED.iloc[:, 17:])
    xs = transformed[:,0]
    ys = transformed[:,1]
    if data_SORTED['Class'].groupby(data_SORTED['Class']).count().shape[0] > 1:
    	number = data_SORTED['Class'].groupby(data_SORTED['Class']).count()[1]
